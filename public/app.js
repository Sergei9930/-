...
socket.on('room_state', (state) => {
  ...
  roundStatus.textContent = state.phase === 'betting' ? `Раунд #${state.roundId}: Ставки` : `Раунд #${state.roundId}: Монета крутится`;

  coinEl.classList.toggle('spinning', state.phase !== 'betting');

  playersList.innerHTML = '';
  state.players.forEach((p) => {
    const li = document.createElement('li');
    const bet = state.bets[p.id] ? ` | ставка: ${state.bets[p.id].amount} на ${state.bets[p.id].side === 'heads' ? 'орла' : 'решку'}` : '';
...
socket.on('round_result', ({ result, winners }) => {
  const winnerText = winners.length ? winners.map((w) => `${w.name} +${w.reward}`).join(', ') : 'Нет победителей';
